...
def test_the_student_t_exception_is_the_worst_cell_not_the_smallest_cohort(
    ...
    _quotes(results_tex, r"under skewed effects on the")
...
def test_input_validation_is_nearly_blind_in_the_interior(results_tex, stress_cells):
    ...
    _quotes(results_tex, r"it is nearly blind")
...
def test_the_figures_exist_and_name_their_table(results_tex):
    """A missing or stale figure is the transcription failure this paper is about.

    The PDFs are committed and regenerated by make_fig*.py; each caption names
    the table it is drawn from.
    """
    figures = REPO_ROOT / "paper" / "bmc" / "figures"
    for filename, label in (("fig1_interval_stress.pdf", "fig:stress"),
                            ("fig2_brackets.pdf", "fig:brackets")):
        path = figures / filename
        assert path.exists() and path.stat().st_size > 0, filename
        _quotes(results_tex, f"figures/{filename}")
        assert f"\\label{{{label}}}" in results_tex
    _quotes(results_tex, r"interval\_stress\_calibration\_summary")
    _quotes(results_tex, r"cutpoint\_crossing\_brackets")
