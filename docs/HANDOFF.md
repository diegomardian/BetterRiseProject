...
| **Crowell multi-section — 2 of 7 blocks read 2026-09-07. NO INTERVAL AT n=2.** | `docs/prereg_crowell_multisection.md` + Amendments 1–2; aggregator `src/reference/jobs/crowell_multisection.py`. §4's DiD, `Δ(gene) − Δ(KRT8)`: **GUCA2A −1.513 and −0.838** across two blocks, MS4A12 −1.371 and −0.759, both same-sign; **CDX2 −0.012 and +0.215, not same-sign**, which is §7's discriminator and at n=2 it discriminates nothing. Below `MIN_STUDIES` the interval is **refused**, not widened. **Amendment 2 is the limit that matters:** KRT8 is the only control-role gene and it is an epithelial keratin — it rose +1.746 against a depth term of +0.356 in block 231 — so it tracks epithelial fraction, and **the DiD does not separate compositional from intrinsic**. Sections 110 and 231 read; §3 fixes 120 next. |
