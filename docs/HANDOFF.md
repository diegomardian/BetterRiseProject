...
| **Crowell multi-section — 4 of 7 blocks read 2026-09-07. INDETERMINATE.** | `docs/prereg_crowell_multisection.md` RESULT + Amendments 1–4; `results/2026-09-07_a9b3d6d/`. **GUCA2A −1.318 [−2.609, −0.027] excludes zero over four blocks — and [−2.195, +0.262] without the one block where the target sits BELOW the negative-probe floor.** Amendment 4, fixed and pushed before the aggregate ran, makes that INDETERMINATE rather than a positive: a below-floor DiD is a bound, not a point. **MS4A12 excluded zero at three blocks and includes it at four**, which is what Amendment 3's multiplicity caution was for. GUCA2A and MS4A12 negative in all four blocks, CDX2/EPCAM not consistently signed — sign agreement, not an effect size, at 5.17× avenue A's width. Amendment 2 still caps everything: KRT8 is an epithelial keratin, so **the statistic does not separate compositional from intrinsic**. Blocks 221, 222, 242 remain. |
