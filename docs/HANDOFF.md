...
| **Crowell multi-section — 6 of 7 blocks read 2026-09-07. §7's PASS, heavily qualified. 242 blocked by Zenodo 504.** | `docs/prereg_crowell_multisection.md` RESULT + Amendments 1–5; `results/2026-09-07_daa3cbd/`. **GUCA2A −1.378 [−2.046, −0.709] and MS4A12 −1.272 [−2.053, −0.492] exclude zero over six blocks; CDX2 +0.448 [−0.020, +0.915] and EPCAM +0.242 do not.** CDX2 is **0.020** from clearing zero in the direction opposite the target — Amendment 7's state, one block away. §7's pass: the target falls, the discriminator does not. **All guards checked**: Amendment 4's leave-out holds (excludes zero with AND without the below-floor block, where at n=4 it did not), and Amendment 5's near-floor block is removable without changing either conclusion. **What it is not:** not silencing, not per-cell (feasibility §7), and **not a separation of compositional from intrinsic** — Amendment 2, which caps this at any n. §6 disclaims n=3 and n=4 and says nothing about n=5, which is not an endorsement; width is 4.03× avenue A's. No multiplicity control. **Only 242 remains and it could not be downloaded** — Zenodo 504'd on every endpoint on 2026-09-07, including files that had worked hours earlier. §3's order was followed exactly, so this is a download failure at a named file, not a stopping point. Retry it. |
