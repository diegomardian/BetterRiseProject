...
| **D2 — CLOSED, and null** | Gate PASS (n=624, IQR 2.96), locked, implemented and run the same day. `results/2026-09-07_b42a2cd/`: **PFI ABSOLUTE 0.9697 [0.8564, 1.0979]**, DSS 1.0013, OS 0.9137 — **every interval contains 1**, PH clean, 11.27 events/df. A well-conditioned null, not a failed run. The `PURITY-SOURCE SENSITIVE` label is §6 applied as written to two nulls straddling 1 and was not re-decided after the fact (§6c). §6a held: ABSOLUTE drops 107 participants whose GUCA2A is *lower* than those it keeps. `docs/prereg_d2_survival.md` §8. |
...
| **Roadmap** | Seven avenues re-scoped; the H&E item is a separate molecular-prediction data gate, not a validation of avenue A. `docs/NEXT_AVENUES.md`. |
| **Process** | `CONTRIBUTING` §3 (never `git add -A`; `git commit --only`) and §6 (ML is measurement infrastructure, or a prediction against an independently measured endpoint). **Invariant 11 is merged** (`89a16df`) — no transcript-derived label may claim its own programme, enforced at specification validation in `src/common/label_provenance.py`. |
...
| **Crowell multi-section — COMPLETE, 7 of 7 blocks, 2026-09-07. §7's PASS, heavily qualified.** | `docs/prereg_crowell_multisection.md` RESULT + Amendments 1–7; `results/2026-09-07_da1f46b/`. **GUCA2A −1.307 [−1.872, −0.741] and MS4A12 −1.241 [−1.874, −0.609] exclude zero, negative in 7 of 7 blocks; CDX2 +0.327 and EPCAM +0.234 do not.** Width 3.01× avenue A's. **All guards checked**: §5 rule 3 passes on all seven, Amendment 4's leave-out holds for the target, Amendment 6's holds for the discriminator. **Amendment 7's state did not arrive and the seventh block is why** — CDX2's lower bound ran −0.251, −0.156, **−0.020**, −0.151, so at six blocks it was 0.020 from clearing zero and the seventh pushed it back. That is the argument for §3's fixed order. **What it is not:** not silencing, not per-cell (§7), **not a separation of compositional from intrinsic** (Amendment 2, at any n), not a replication of avenue A. |
...
| **H&E foundation-model ML — CONDITIONAL DATA GATE; not yet an analysis** | This is an orthogonal morphology-to-molecular-label project, not a validation of avenue A or a response to Crowell's transcript sensitivity limit. It starts only if a public or approved source supplies full-resolution H&E plus a **specimen-exact** join to an independently assayed, pre-specified molecular endpoint. Polyp subtype/dysplasia labels read from the same H&E are diagnostic-label imitation and cannot be the biological result. All image tiles must remain patient-contained in train/validation/test splits; the abstention threshold belongs to validation, with coverage and selective error reported once on held-out patients (and held-out site when possible). If the join, endpoint prevalence, or split is unavailable, record **NO SUBSTRATE**; do not substitute a morphology-derived endpoint. `docs/NEXT_AVENUES.md` item 2b. |
