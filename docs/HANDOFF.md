...
| **Crowell multi-section — PRE-REGISTERED 2026-09-07, nothing downloaded** | `docs/prereg_crowell_multisection.md`. Tests whether §2's single-patient REF→lesion fall holds across patients. Statistic is a difference in differences on the detection scale, `Δ(gene) − Δ(KRT8)`, one number per block, Student-t over patients — **not a band**, because with ACTB absent there is one control gene and a band of one is a point. Seven blocks exist; **they do not all fit on `/projectnb`** (6.97 GB remaining against 7.85 GB free), so §3 fixes a one-at-a-time order with the h5ad deleted after its table is written. **`DiD(CDX2)` is the row that matters** — a negative `DiD(GUCA2A)` alone is consistent with the whole differentiation programme moving, which is not the claim. Below 3 patients no interval is reported. |
| **A2 · same-cohort MxIF — STEP 1 INVENTORIED 2026-09-07; no outcome read** | `docs/a2_mxif_inventory.md`, fixed source rows `config/a2_mxif_regions.csv`, job `src/reference/jobs/a2_mxif_inventory.py`. The public MILWRM Zenodo release has **42 processed 26-channel pixel NPZ regions across 15 HTA11 participants** and includes CDX2/OLFM4/SOX9/MUC5AC/DAPI, but **no cell segmentation**. Supplementary Table 1 labels 38 regions; four archive-only regions remain unlabelled. All 15 patients match Avenue A's validation metadata, 11 with both scRNA arms, but this is participant-level rather than specimen-exact. Only **8** MxIF patients have a labelled normal/tumour within-batch pair, and only **3 are conventional adenomas** (5 SSL). Thus the public product licenses a pixel-pipeline pilot, not the proposed per-cell crypt-position equivalence test. The MANDO-derived cell table remains behind Synapse access or is absent; do not choose a replacement outcome after seeing this constraint. |
