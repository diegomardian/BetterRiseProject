...
| **Crowell multi-section — 3 of 7 blocks read 2026-09-07. NO CLAIM.** | `docs/prereg_crowell_multisection.md` + Amendments 1–2; aggregator `src/reference/jobs/crowell_multisection.py`. §4's DiD, `Δ(gene) − Δ(KRT8)`: at three blocks **GUCA2A −0.966 [−2.195, +0.262] includes zero — §7's NO CLAIM branch**, and at 8.07× avenue A's width it may not be quoted as a negative either. **MS4A12 −0.951 [−1.855, −0.047] excludes zero and §7 has no branch for that** (Amendment 3): same mean, tighter spread, four informative genes with no multiplicity control, and `P(≥1 excludes | global null) = 0.19`. CDX2 +0.265 does not fall, which is the discriminator holding. **Amendment 2 is the limit that matters:** KRT8 is the only control-role gene and it is an epithelial keratin — it rose +1.746 against a depth term of +0.356 in block 231 — so it tracks epithelial fraction, and **the DiD does not separate compositional from intrinsic**. Blocks 231, 110 and 120 read; §3 fixes 210 next. 120's own per-cell verdict is NOT ASKABLE (target below the bar in every domain) and it still enters the DiD — §5's gate is the control, and excluding it would select on the outcome. |
