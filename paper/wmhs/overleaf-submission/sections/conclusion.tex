\iffull
Five of this paper's statistics could not fail. A recovery curve in which the
...
sits above its own attainable bound still reports clean; a premise check
reading a control detected in 99\% of cells reports comparable arms while those
same controls shift by up to $0.71$ on log$_2$ expression; and the percentile
bootstrap every one of those readings ends in is $0.82\times$ the width it claims
at $n = 10$, under a normal-mean approximation containing no data. Each looked like a
...
specifically, and the answer generalises past our instance: specify an empirical reference and audit its difference from the estimate.
A uniformly zero residual warrants an algebraic check for functional reuse.
In our patient simulator it exposes equivalence between methods whose recovery
curves agree to machine precision. Keep error against the requested parameter,
interval coverage and calibration of the decision rule as separate evaluations.
