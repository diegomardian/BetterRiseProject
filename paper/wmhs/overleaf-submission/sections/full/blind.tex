...
pipeline. To bound its prevalence we pre-registered a search and a
three-way classification rule, then audited seven public repositories in
trial simulation, target-trial emulation and plasmode simulation
(Appendix~\ref{app:prevalence}). 	extbf{None reuses the functional: 0 of 7.}
Six never reach the question, computing the reference from generating
parameters --- the practice we recommend, already the norm. The seventh is
instructive: its shipped residual of $4.0\times10^{-14}$ reads as reuse and is
not, because its benchmark data are noiseless and exactly linear; adding
outcome noise separates the two functionals at $0.125$. A tolerance screen
alone would have misclassified it, which is why the audit resolves
near-equalities algebraically. We therefore claim the failure is possible
and cheap to exclude, not that it is widespread.
