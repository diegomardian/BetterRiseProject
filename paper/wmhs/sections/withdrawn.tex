% Two guards that could not fire. Sits inside §4 (sec:calibration) with no
% \section of its own — see PATCHES.md E1/G4.

The two defective validation statistics in \S\ref{sec:calibration} are not
isolated. Our working rule holds that \textbf{a check unable to fail is worse
...
Five others were plain implementation errors and sit in
Appendix~\ref{app:withdrawn} as record rather than finding.
...
Our repair inherited the defect. The repaired diagnostic reports the attainable
bound, but takes the worst correlation and the largest bound as
\emph{independent} maxima over the two arms, so the statistic and its bound
sometimes come from different arms. Correcting the pairing takes the
reachability flag with it, and at the rarest rung that flag called the test
capable of firing on \textbf{64 of 64} combinations where 16 is correct.
