...
coverage and abstention rules require separate evaluation. We argue it
matters more, not less, for learned generative patient models: there the
realised reference has no closed form and must itself be estimated, so
functional reuse becomes the default wiring, the equality degrades to an
approximate one a boolean screen cannot see, and pre-specifying the
reference becomes the substantive modelling decision.
\end{abstract}
