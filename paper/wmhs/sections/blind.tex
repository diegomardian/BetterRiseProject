...
\paragraph{The procedure, in four steps.} Before the sweep, specify $T(D)$,
its target population and observation process. Then report
$\max_D|\hat\theta(D)-T(D)|$ per design cell at a fixed tolerance, counting
non-finite outputs separately. Read exact equality as a prompt for algebra, not
as estimator failure, and read departure as no quality certificate. Finally,
report its scale relative to the generator's sampling error. At a null requested
effect use differences, not recovery ratios. Table~\ref{tab:matrix} shows why
the first step matters: changing the reference changes which estimator is
