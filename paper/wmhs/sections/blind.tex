...
the summary the generator just computed.\iffull{} Wiring the estimator to the
generator's own stratum means is the shortest path to an arm that runs, and it is
the first arm most pipelines get.\fi{} This is not a strawman we built to knock
down: it is the arm we shipped.
...
precisely the mistake the column prevents and reading the code did not.\iffull{}
Whether an estimator shares sufficient statistics with the generator
is not visible in the code. It is visible in one column.\fi
...
as the estimate improves. That curve is about the estimator.\iffull{} The unadjusted
...
An exact zero proves that \emph{this comparison} is blind: if
$\hat\theta \equiv \theta_{\text{realised}}$ the ratio is a functional of the
generator, by algebra, so it carries no information about the estimator. It does
not follow that the validation is uninformative --- simulation can still measure
bias, variance and coverage against a population parameter the sample does not
reproduce exactly. The converse fails, in
...
\]
a ratio of median absolute deviations --- the estimator's median residual in
units of the generator's own sampling error, not a share of any variance. OLS
scores $\rho$ between $0.129$ and $0.137$ at every cohort size from $100$ to
$5{,}000$, with no trend in $n$: its median residual is about an eighth of the
generator's, and it passes the boolean check. Cross-fitted IPW falls
$1.69 \rightarrow 0.23$ by $n = 5{,}000$. The unadjusted difference scores
$52.8$ and is the only estimator here whose residual exceeds the generator's.
$\rho = 0$ is the boolean check as a special case; $0 < \rho \ll 1$ is the
regime it cannot see.
\fi
...
1{,}200 replicates $\times$ 6 seeds; draws leaving an arm with no event are
dropped, which costs nothing below $49\%$ censoring and $463$ of $1{,}200$ in
...
latent event times it degrades to nothing}: at $n = 3{,}000$ the residual runs
...
curve takes the
closed form $1/(1-s)$ exactly at the abstention point --- $5.000$, $2.000$ and $1.333$ at the three
