...
\paragraph{The one-line check.} Specify the reference functional, its target
population and its observation process before running the boxed screen. Report
non-finite outputs separately; otherwise a NaN makes the assertion pass.
Set \texttt{tol} explicitly in outcome units and report the maximum residual per
design, not just a pooled median. Near-zero residuals motivate an algebraic
check; nonzero residuals do not certify calibration. At a null requested effect,
use differences rather than recovery ratios.
