...
\paragraph{The prevalence bound collapses at both ends.} $\sqrt{3p(1-p)}$ goes to
zero as $p \to 1$ just as it does as $p \to 0$. On the coarsest resolution every
epithelial cell is called mature, prevalence is exactly $1.000$, the bound is
exactly zero, and the correlation is undefined on all 64 patient-by-axis rows ---
the check reports nothing at all. Degenerate labels are invisible to this check
at either extreme, and the two extremes are the coarsest and the finest
resolution we ran.
