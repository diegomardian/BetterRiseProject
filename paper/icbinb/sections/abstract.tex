...
abstention rule. All four came back clean, and we read that as evidence the
measurement was sound. \textbf{A check unable to fail is worse than
no check, because it turns an absence of evidence into a green light}, and four
of ours could not fail.

Two of the four generalise past our data and are provable rather than merely
observed. A threshold on the correlation between sequencing depth and a
\emph{binary} cell label of prevalence $p$ cannot fire at all once the threshold
exceeds $\sqrt{3p(1-p)}$ --- so on rare cell types, the populations worth
resolving, the diagnostic reports clean whatever the data does. On two cohorts'
published subtype labels that is 570 rows, 250 of them above half the
correlation their prevalence permits, and none flagged. And a recovery curve cannot see
...
finding that neither its own calibration nor an ablation supports the cutpoint we
committed to.
\end{abstract}
